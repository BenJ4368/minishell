...
/*   Updated: 2023/11/23 15:32:00 by ssalor           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_split_unquoted_write_word(char *dest, const char *src, char c)
{
	...
	i = 0;
	while (((src[i] == c && check_in_quotes(src, i, 0, 0) == 0) || src[i] == '\0') == 0)
	{
		dest[i] = src[i];
		i++;
	}
...
int	ft_split_unquoted_write(char **tab, char const *s, char c)
{
	...
	while (s[i])
	{
		if (((s[i] == c && check_in_quotes(s, i, 0, 0) == 0) || s[i] == '\0') == 1)
			i++;
		else
		{
			j = 0;
			while (((s[i + j] == c && check_in_quotes(s, i + j, 0, 0) == 0) || s[i + j] == '\0') == 0)
				j++;
...
int	ft_split_unquoted_count_words(char const *s, char c)
{
	...
	while (s[i])
	{
		if (((s[i + 1] == c && check_in_quotes(s, i + 1, 0, 0) == 0) || s[i + 1] == '\0') == 1
			&& ((s[i] == c && check_in_quotes(s, i, 0, 0) == 0) || s[i] == '\0') == 0)
			count++;
		i++;
	}
	return (count);
